...
const fetchJson = async () => {
  const data = await fetch("./data.json")
    ...
    .then((data) => {
      ...
      // TOPICS
      //  <p
      //     class="bg-screamin-green-400 pt-4 pb-4 p-8 rounded-2xl shadow-blue-400 shadow-sm border-2 border-purple-300 font-semibold text-3xl hover:scale-105 hover:duration-500 xl:text-2xl xl:p-6 xl:pb-2 xl:pt-2 flex items-center md:text-lg ">
      //     web developement
      //   </p>
      const topics = data.topics;
      topics.forEach(ele => {
        const p = document.createElement('p');
        p.classList = 'bg-screamin-green-400 pt-4 pb-4 p-8 rounded-2xl shadow-blue-400 shadow-sm border-2 border-purple-300 font-semibold text-3xl hover:scale-105 hover:duration-500 xl:text-2xl xl:p-6 xl:pb-2 xl:pt-2 flex items-center md:text-lg';
        p.innerText = ele
